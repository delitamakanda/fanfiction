<template>
    <div v-if="obj_fanfic && obj_fanfic.author && obj_fanfic.author.username">
        ...
        <div class="w-full">
            <div class="flex flex-col justify-between leading-normal">
                <div class="mb-8">
                    ...
                    <p class="text-gray-700 text-base" v-if="obj_fanfic.description && !obj_fanfic.fanfic_is_scraped">
                    ...
                    <p class="text-gray-700 text-base">
                        {{ obj_fanfic.genres }} - {{ obj_fanfic.classement }} - {{ obj_fanfic.publish | date }} 
                        - id: {{ obj_fanfic.id }} - {{ obj_fanfic.total_likes }} {{'like' | pluralize(obj_fanfic.total_likes) }}
                        - {{ obj_fanfic.views }} {{ 'view' | pluralize(obj_fanfic.views) }} - 
                        {{ $t('message.publishedAtLabel') }} : {{obj_fanfic.publish | date }} <span
                            v-if="obj_fanfic.updated"> - {{ $t('message.miseAJourLabel') }} : {{ obj_fanfic.updated | date }}</span> 
                            - {{ commentsCount }} {{ 'review' | pluralize(commentsCount) }} -
                        <router-link :to="{ name: 'Reviews', params: { isPrivate: false, fanficId: obj_fanfic.id } }">
                    ...
                    <template v-if="user && user.id != null" class="flex items-center">
                        ...
                        <button v-if="followUser" v-model="getAuthorFollowed" type="button" @click="DisFollowAuthor"
                            class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-full">
                            {{$t('message.NotFollowAuthorText') }}</button>

                        <button v-if="!followStory" v-model="getStoriesFollowed" type="button" @click="followFanfic"
                            class="bg-teal-500 hover:bg-teal-700 text-white font-bold py-2 px-4 rounded-full">
                            {{ $t('message.followFanficText') }}</button>
                        ...
                        <button type="button"
                            ...
                            v-model="getUserLiked" @click="favorite" v-if="!like">
                            <svgicon icon="mood-happy-solid" width="22" height="18" color="#fff"></svgicon> 
                            {{ counter }}
                        </button>

                        <button type="button"
                            class="inline-flex items-center bg-red-500 hover:bg-red-darker text-white font-bold py-2 px-4 rounded-full"
                            v-model="getUserLiked" @click="unfavorite" v-if="like">
                            <svgicon icon="mood-sad-solid" width="22" height="18" color="#fff"></svgicon> 
                            {{ counter }}
                        </button>
                    </template>
                ...
                <div class="flex items-center">
                    <router-link :key="obj_fanfic.id"
                        :to="{ name: 'ShowUserFanfic', params: { username: obj_fanfic.author.username, slug: obj_fanfic.slug, id: obj_fanfic.id, isPrivate: false }}">
                        <avatar :email="obj_fanfic.author.email" class="h-16 w-16 md:h-10 md:w-10 rounded-full mx-auto"
                            :alt="obj_fanfic.author.username" />
                    </router-link>
                    <div class="text-sm">
                        <router-link :key="obj_fanfic.id"
                            :to="{ name: 'ShowUserFanfic', params: { username: obj_fanfic.author.username, slug: obj_fanfic.slug, id: obj_fanfic.id }}">
        ...
        <div class="clearfix" v-if="!obj_fanfic.fanfic_is_scraped">
            <div class="float-right">
                <div class="inline-block relative w-64">
                    <select v-model.lazy="selecteur"
                        class="block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline">
                        <option v-for="(chapter, i) in chapters" :key="chapter.id" :value="chapter.id">
                            {{ i + 1 }} - {{ chapter.title }}</option>
                    </select>
        ...
        <Form v-if="!obj_fanfic.fanfic_is_scraped" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" title=""
            :operation="writeComment" :valid="valid">
            <div class="mb-4">
                <label class="block tracking-wide text-grey-darker text-xs font-bold mb-2" for="name">
                    {{ $t('message.UsernameOrPseudoLabel') }}
                </label>
                <Input name="name" v-model="form.name" :placeholder="$t('message.UsernameOrPseudoLabel')"
            ...
            <div class="mb-4">
                ...
                <Input name="email" v-model="form.email" :placeholder="$t('message.emailPlaceholderLabel')"
                    maxlength="255" />
            </div>
            <div class="mb-4">
                ...
                <Input type="textarea" name="body" v-model="form.body"
                    :placeholder="$t('message.formContactMessageLabel')" rows="6" required />
            </div>
        ...
        <div class="w-full my-8 text-center">
            <button type="button" @click.once="feedback"
                class="bg-teal-500 hover:bg-teal-500 text-white font-bold py-2 px-4 rounded-full">
                {{ $t('message.signalerLabel') }}</button>
            <button v-if="!obj_fanfic.fanfic_is_scraped" type="button" @click="printFanfic"
                class="bg-teal-500 hover:bg-teal-500 text-white font-bold py-2 px-4 rounded-full">
                {{$t('message.formatPDFLabel') }}</button>

            <button v-if="obj_fanfic.fanfic_is_scraped && obj_fanfic.link_fanfic"
                @click="openLink(obj_fanfic.link_fanfic)" type="button"
                class="bg-teal-500 hover:bg-teal-500 text-white font-bold py-2 px-4 rounded-full">
                {{ $t('message.linkLabel') }}</button>
        </div>

        <div v-if="obj_fanfic.recommended_fanfics && obj_fanfic.recommended_fanfics.length"
            ...
            <div class="flex flex-wrap flex-col md:flex-row">
                <div v-for="f in obj_fanfic.recommended_fanfics"
                    class="flex flex-wrap bg-white border-b border-blue-tial-100 md:w-1/2 lg:w-1/3" :key="f.id">
