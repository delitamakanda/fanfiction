                ...
                class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <Input
                            type="textarea"
                            name="description"
                            v-model="get_chapter.description"
                    ...
                    <div class="mb-4">
                        ...
                        <trumbowyg v-model="get_chapter.text"></trumbowyg>
                    </div>
                    <Input
                        type="hidden"
                        name="fanfic"
                        v-model="get_chapter.fanfic" />
                    <button
                        type="button"
                        @click="closeChapterEditiong"
                        class="secondary inline-block align-baseline font-bold text-sm text-blue hover:text-blue-darker">
                        Retour à la l'histoire
                    </button>
                    <button
...
export default {
    ...
    methods: {
        ...
        editFanfic () {
            ...
            if (this.isEditingChapter) { this.isEditingChapter = false }
        },
        async editChapter (id) {
            this.get_chapter = await this.$fetch(`chapters/${id}`)
            this.isEditingChapter = true
            if (this.isEditingFanfic) { this.isEditingFanfic = false }
            this.chapter_id = id
        },
        closeChapterEditiong () {
        ...
        async editingChapter () {
            const result = await this.$fetch(`chapters/${this.chapter_id}`, {
                method: 'PUT',
                body: JSON.stringify({
                    title: this.get_chapter.title,
                    description: this.get_chapter.description,
                    text: this.get_chapter.text,
                    fanfic: this.get_chapter.fanfic,
