                    ...
                    <li @mouseover="survol = true" @mouseleave="survol = false"><svgicon icon="question" width="22" height="18" color="#4dc0b5"></svgicon> Aide
                        <transition name="fade" mode="out-in">
                            <ul class="bg-white p-4 z-10 shadow " v-if="survol" @click="survol = false">
                                ...
                                <li><a href="#" class="text-teal hover:text-teal-darker" @click="openModalContact">{{ $t('message.contactLabel') }}</a></li>
                            </ul>
                        </transition>
                    </li>
                </ul>
...
export default {
    ...
    methods: {
        ...
        ...mapActions('other', ['getNotifications', 'clearNotifications']),
        toggle (){
            this.open = !this.open
        },
        /*async logout () {
