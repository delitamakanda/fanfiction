...
def compute_recommendations(user_id):
	...
	recs = (UserFavorite.objects.filter(user_id__in=similar_users).exclude(fanfiction_id__in=user_favorites).values('fanfiction_id').annotate(score=Count('fanfiction_id')).order_by('-score')[:5])

	fanfictions_ids = [r['fanfiction_id'] for r in recs]
