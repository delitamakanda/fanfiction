          ...
          <div class="px-6">
            <div class="flex flex-wrap justify-center">
              <div class="w-full lg:w-3/12 px-4 lg:order-2 flex justify-center">
                <div class="relative">
                  <img :alt="profile.username" v-if="profile.photo" :src="profile.photo"
                    class="shadow-xl roundedCircle align-middle border-none">
                  <Avatar v-else :email="profile.email" ref="avatar"
                    class="shadow-xl roundedCircle align-middle border-none " />
                </div>
              </div>
              ...
              <div class="w-full lg:w-4/12 px-4 lg:order-1">
                <div class="flex justify-center py-4 lg:pt-4 pt-8">
                  <div class="mr-4 p-3 text-center">
                    <span class="text-xl font-bold block uppercase tracking-wide text-blueGray-600">{{ countFanfics
                      }}</span><span class="text-sm text-blueGray-400">
                      {{ $t('message.profilePage.fanficsLabel', countFanfics) }}
                    </span>
                  </div>
                  <div class="mr-4 p-3 text-center">
                    <span v-if="followAuthors"
                      class="text-xl font-bold block uppercase tracking-wide text-blueGray-600">{{ followAuthors.length
                      }}</span><span class="text-sm text-blueGray-400">
                      {{ $t('message.profilePage.favAuthorsLabel', followAuthors.length) }}
                    </span>
                  </div>
                  <div class="lg:mr-4 p-3 text-center">
                    <span v-if="followStories"
                      class="text-xl font-bold block uppercase tracking-wide text-blueGray-600">{{ followStories.length
                      }}</span><span class="text-sm text-blueGray-400">
